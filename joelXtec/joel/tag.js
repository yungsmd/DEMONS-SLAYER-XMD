...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    if (!botAdmin) return m.reply("*мυѕт вє α∂мιη тσ υѕє тнιѕ ¢σммαη∂*");
    if (!senderAdmin) return m.reply("*уσυ αяє ησт α∂мιη*");
    // Extract the message to be sent
    let message = `乂 *ᴀᴛᴛᴇɴᴛɪᴏɴ ᴇᴠᴇʀʏᴏɴᴇ* 乂\n\n*Message:* ${m.body.slice(prefix.length + cmd.length).trim() || 'no message'}\n\n`;
        


    for (let participant of participants) {
      message += `> ❏ᴅᴇᴀʀ@${participant.id.split('@')[0]}\n`;
    }

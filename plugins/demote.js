...
const text = m.body.slice(prefix.length + cmd.length).trim();
    ...
    if (!m.isGroup) return m.reply("*ɢʀᴏᴜᴘ ᴄᴏᴍᴍᴀɴᴅ*");
    ...
    if (users.length === 0) {
      return m.reply("*ᴍᴇɴᴛɪᴏɴ ᴏʀ ǫᴜᴏᴛᴇ ᴀ ᴜsᴇʀ ᴛᴏ ᴅᴇᴍᴏᴛᴇ*");
    }
    ...
    await gss.groupParticipantsUpdate(m.from, validUsers, 'demote')
      .then(() => {
        ...
        m.reply(`*ᴜsᴇʀ ${demotedNames} ᴅᴇᴍᴏᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ ${groupMetadata.subject}*`);
      })
